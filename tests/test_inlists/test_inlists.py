...
class TestFunctions(unittest.TestCase):
    ...
    def test_evaluate_inlist(self):
        self.assertDictEqual(self.inlist, self.correct)
    ...
    def test_evaluate_inlist_str(self):
        ...
        self.assertDictEqual(inlist, self.correct)


    def test_inlist_diff(self):
        self.assertRaises(ValueError, inl.inlist_diff, self.inlist, self.inlist)
        self.assertDictEqual(inl.inlist_diff(self.inlist['controls'], self.inlist['controls']),
                             {'same':dict(self.inlist['controls']), 'changed':{}})

        diff_in1_in3 = {'same': {},
                        'changed': {'initial_mass': (1.0, 3.0), 'initial_z': (0.01, None)}}
        self.assertDictEqual(inl.inlist_diff(self.sub_inlists[0]['controls'], self.sub_inlists[2]['controls']), diff_in1_in3)
    ...
    def test_compare_inlist(self):
        ...
        self.assertEqual(capturedOutput.getvalue(), ("left : ../data/inlists/inlist_3\n"
                                                     "right: ../data/inlists/inlist_1\n"
                                                     "\n"
