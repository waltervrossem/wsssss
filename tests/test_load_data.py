...
class TestLoadData(unittest.TestCase):
    def test_History(self):
        hist = ld.History(os.path.join(test_data, '0', 'LOGS', 'history.data'))
        hist.dump()

        hist_dill = ld.History(os.path.join(test_data, '0', 'LOGS', 'history.data.dill'))
        np.testing.assert_array_equal(hist.data, hist_dill.data)
        self.assertDictEqual(hist.header, hist_dill.header)
        del hist_dill

        hist_reload = ld.History(os.path.join(test_data, '0', 'LOGS', 'history.data'), save_dill=True, reload=True)
        ...
        hist_cols = ld.History(os.path.join(test_data, '0', 'LOGS', 'history.data'), keep_columns=['model_number', 'center_he4'])
    ...
    def test_Profile(self):
        prof = ld.Profile(os.path.join(test_data, '0', 'LOGS', 'profile1.data'))
        hist = ld.History(os.path.join(test_data, '0', 'LOGS', 'history.data'))
        ...
        prof = ld.Profile(os.path.join(test_data, '0', 'LOGS', 'profile1.data'), load_GyreProfile=True)

    def test_GyreSummary(self):
        gsum = ld.GyreSummary(os.path.join(test_data, '0', 'gyre_out', 'profile10.data.GYRE.sgyre_l'))
        self.assertEqual(7, len(gsum.data[gsum.data['l'] == 0]))
        self.assertEqual(236, len(gsum.data[gsum.data['l'] == 1]))

    def test_GyreProfile(self):
        prof = ld.Profile(os.path.join(test_data, '0', 'LOGS', 'profile1.data'))
        gprof = ld.GyreProfile(os.path.join(test_data, '0', 'LOGS', 'profile1.data.GYRE'))
    ...
    def test_GyreMode(self):
        gsum = ld.GyreSummary(os.path.join(test_data, '0', 'gyre_out', 'profile10.data.GYRE.sgyre_l'))
        gmode = ld.GyreSummary(os.path.join(test_data, '0', 'gyre_out', 'profile10.data.GYRE_l0_00005_np+9_ng+0.mgyre'))
        np = 9
        ng = 0
    ...
    def test_load_gss(self):
        hist = hist = ld.History(os.path.join(test_data, '0', 'LOGS', 'history.data'))
        gss = ld.load_gss(hist)
        self.assertEqual(11, len(gss))

    def load_modes_from_profile(self):
        gsum = ld.GyreSummary(os.path.join(test_data, '0', 'gyre_out', 'profile10.data.GYRE.sgyre_l'))
        prof = ld.Profile(os.path.join(test_data, '0', 'LOGS', 'profile10.data'))
