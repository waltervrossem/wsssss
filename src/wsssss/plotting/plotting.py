...
def make_propagation_CouplingData(cd, p=None, ax=None, add_legend=True, show_approx=False):
    ...
    if p is not None:
        if p.header['model_number'] == cd.model_number:
            ...
            print(
                f'profile model {p.header["model_number"]} number does not match CouplingData model number {cd.model_number}.')
...
def check_approx_PQ(hist, prof, xname='s', linear_grad=False, axes=None, same_coeffs='', grd_type=0, show_coeffs=True):
    ...
    if xname == 's':
        ...
        xlim = (0.95 * min(hist.data.r_1[i_hist], hist.data.r_2[i_hist]),
                1.05 * max(hist.data.r_1[i_hist], hist.data.r_2[i_hist]))
    else:
        ...
        if i == 0:
            ...
            if grd_type in [0, 1]:
                ...
                if linear_grad:
                    ...
                    axb.set_ylabel(
                        r'$\frac{1}{P}\frac{\mathrm{d}}{\mathrm{d} s} P$, $\frac{1}{Q}\frac{\mathrm{d}}{\mathrm{d} s} Q$')
                axb.set_ylim(b_ylims)
            elif grd_type == 2:
                ax.set_ylabel(r'$\frac{\mathrm{d}}{\mathrm{d} s} \ln \frac{P}{Q} \; \frac{s+s_0}{s_0-s}$')
                if linear_grad:
                    axb.set_ylabel(
                        r'$\frac{\mathrm{d}}{\mathrm{d} s}\; \frac{P}{s_0-s}$, $\frac{\mathrm{d}}{\mathrm{d} s}\; \frac{Q}{s+s_0}$')
                else:
                    axb.set_ylabel(
                        r'$\frac{\mathrm{d}}{\mathrm{d} s}\; \ln \frac{P}{s_0-s}$, $\frac{\mathrm{d}}{\mathrm{d} s}\; \ln \frac{Q}{s+s_0}$')
                ax.set_ylim(ylims)
                axb.set_ylim(b_ylims)
            elif grd_type == 3:
                ax.set_ylabel(r'$\frac{\mathrm{d}}{\mathrm{d} s} \ln \frac{P}{Q} \; \frac{s+s_0}{s_0-s}$')
