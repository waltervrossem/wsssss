...
#
# class MesaData:
#     """Reads a Mesa history or profile file and lazily loads the data."""
#     def __init__(self, path: str, save_dill=False, reload=False, verbose=True, filetype=None, nanval=-1e99,
#                  nanclip=None, index_name='profiles.index', keep_columns='all'):
#         self.path = os.path.abspath(path)
#         self.LOGS = os.path.dirname(self.path)
...
#             self.fname = os.path.basename(self.path)
#         else:
#             raise FileNotFoundError("path must lead to a history or profile file. Absolute path is:" +
#                                     "\n"+os.path.abspath(self.path))
#
#         if self.path.endswith('.dill'):
#             self.dill_path = path
#         else:
#             self.dill_path = os.path.join(self.LOGS, self.fname+'.dill')
#
#         if os.path.isfile(self.dill_path) and not reload:
#             with open(self.dill_path, 'rb') as handle:
#                 try:
...
#                 mesa_or_gyre = 'gyre'
#             else:
#                 raise ValueError(f'Unable to determine mesa or gyre filetype of \n {self.path}.')
#         else:
#             mesa_or_gyre = {'history':'mesa', 'profile':'mesa', 'mesa':'mesa', 'gyre':'gyre'}[filetype]
#
#         if mesa_or_gyre == 'mesa':
#             if 'model_number' in self.header.keys():
#                 self.type = 'profile'
#             else:
#                 self.type = 'history'
#
#             self.index_path = os.path.join(os.path.dirname(self.path), self.index_name)
#
#             try:
#                 index = np.genfromtxt(self.index_path, skip_header=1, dtype=int)
#                 if index.shape == (3,):
...
#             try_to_get = ['model_number', 'num_zones', 'star_mass', 'star_age', 'Teff', 'photosphere_L',
#                           'center_h1', 'center_he4', 'date']
#             return 'MESA profile data file at {}'.format(self.path) + '\n' + str({key: self.header[key] for key in try_to_get})
...
#             return 'MESA history data file at {}'.format(self.path) + '\n' + star_info
#         elif self.type == 'gyre':
#             return 'GYRE data file at {}'.format(self.path)
#         else:
#             return 'Unknown type at {}'.format(self.path)
#
#     def __len__(self):
#         return len(self.data)
#
...
#         """Returns the profile number, model number, and model index"""
#         if hasattr(profile_nums, '__len__'):
#             if isinstance(profile_nums, MesaData):  # single instance of MesaData
#                 profile_nums = [profile_nums]
#         else:
#             profile_nums = [profile_nums]
#         if isinstance(profile_nums[0], MesaData):
#             profile_nums = [p.profile_num for p in profile_nums]
#         idxs = np.where(np.in1d(self.index[:, 2], profile_nums))
#         model_nums = self.index[:, 0][idxs]
#         data_idx = np.where(np.in1d(self.data['model_number'], model_nums))[0]
...
class GyreProfile:
    def __init__(self, path):
        ...
        self.header = {'num_zones': num_zones, 'star_mass': mass, 'star_radius': radius, 'star_luminosity': luminosity,
                  'version': version}
        self.columns = ('zone', 'radius', 'mass', 'luminosity', 'pressure', 'temperature', 'density', 'grad_T', 'brunt_N2',
               'gamma1', 'grad_ad', 'nu_T', 'opacity', 'opacity_partial_T', 'opacity_partial_rho', 'total_energy_generation',
